# Atlas Memory docs

- [Why Atlas](why-atlas.md) — why adopt or migrate
- [Cursor default layer](cursor-default.md) — Atlas as where-to-look / what-to-remember for Cursor
- [Artigo (revista)](revista-atlas.md) — o que é e o que faz, tom de revista de informática
- [Quickstart](quickstart.md)
- [Migration guide](migration.md) — bring existing projects into Atlas
- [Concepts](concepts.md)
- [Adapters](adapters.md)
- [Security](security.md)
- [Team sync](team-sync.md)
- [Protocol Specification](protocol.md) — ARP v1 normative spec (MUST/SHOULD/MAY)
- [Life Specification](life-spec.md) — temporal memory, entities, session continuity
- [Whitepaper](article.md) — ARP technical essay
- [MCP](mcp.md)
- [Atlas Life](life.md) — personal memory + temporal drawers
- [Atlas Chat](atlas-chat.md) — desktop DeepSeek client
- [Publishing](publishing.md)
